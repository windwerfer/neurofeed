...
| Trust graphs | Dart unit | `test/trust_graphs_test.dart`, `test/trust_dirty_test.dart`, `test/nerd_sheet_test.dart` | those files | Chip defaults/persist; dirty skip epoch/audio/guard; held-back gray; verdict priority; More glued; viewport 75 s Follow 15–300; Blink/Jaw live ring; calibrating hides graphs; nerd sheet piles / band stack / (i) rows; guard block omitted if lane did not run | Visual Follow slide **cannot** |
