...
def score_feature(
    ...
    *,
    ...
    sep = roc_auc(play_vals, labels_play)
    base["sep"] = round4(sep)
    ...
    delta_abs = np.asarray(corpus.get("delta_abs", corpus.get("band_delta", play_vals)))[
        play
    ]
    ...
    for p in percentiles:
        thr = threshold_at_percentile(baseline, p)
        if role == "reward":
            decisions = uptrain_in_target(play_vals, thr)
            ...
            la = label_align(warn, labels_play)
            primary = la if la is not None else wr
            # For guard sweeps prefer higher label_align; if using warn_rate alone,
            # prefer rates closer to 0.15 (invert distance as primary for best_p)
            if la is None:
                primary = 1.0 - min(1.0, abs(wr - 0.15) / 0.15)
            row = {
                "p": p,
                "threshold": round4(thr),
                "warn_rate": round4(wr),
